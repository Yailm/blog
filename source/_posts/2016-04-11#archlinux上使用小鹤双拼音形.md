...
字库的获得一般在小鹤群，也可以从飞扬输入法中导出，由于词库也不是一直是固定的。
散步的鹤（也是就创造者）会不定时的更新词库。所以写了个难使的perl脚本来更新字典，前提是字典被放到了/tmp目录下了
...
使用小鹤双拼的时候偶尔会卡字，不清楚字是怎么拆分的，所以反查也是很重要的。
一开始没有注意到rime可以设置反查，就又手撸了脚本来查询，效果也不错，而且支持整句，合并了上面的代码
